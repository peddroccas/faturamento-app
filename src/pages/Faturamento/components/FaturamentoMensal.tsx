  ...
  return (
    <div className="m-2 flex w-11/12 flex-1 flex-col items-center justify-center rounded-3xl bg-bluesr-500 p-4">
      <header className="grid w-full grid-cols-3 items-center">
        ...
        <h2
          className={`transform text-center text-2xl font-bold text-aliceblue transition-all ${switchTransition ? 'scale-95 opacity-0' : 'scale-100 opacity-100'}`}
        >
          Faturamento {table}
        </h2>
      </header>
      <article
        className={`my-4 flex w-full flex-1 items-center justify-center rounded-2xl bg-aliceblue p-2 transition-all ${switchTransition ? 'scale-95 opacity-0' : 'scale-100 opacity-100'}`}
      >
        <Table
